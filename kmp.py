...
def KMP_Matcher(text,pattern):              # KMP matcher function
    ...
    prefix_fun = prefix_function(pattern)              # generate prefix function for the pattern
    q = 0
    for i in range(1,m+1):
        ...
        if q == n:                                      # if q is equal to the length of the pattern, it means that the pattern has been found.
            print("Pattern occurs with shift",i-n)     # print the index, where first match occours.
            flag = True
            q = prefix_fun[q]
    if not flag:
        print('\nNo match found')

KMP_Matcher('aabaacaadaabaaba','baa')              # function call, with two parameters,text and pattern
